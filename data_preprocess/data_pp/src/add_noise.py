...
for name in files:
	if name.startswith('cropped'):
		num_images +=1
print("The number of cropped images: ", num_images)
...
for time in range(times_noise):	
	for name in files:
		if name.startswith('cropped'):
			...
			faces.append(debris[2][1:])
			orientations.append(debris[3][1:])
			for n in range(num_noise):
				x_pxl = random.randint(0,size_image-1)
				y_pxl = random.randint(0,size_image-1)
